...
# any of the repository's own work.
#
#   bash doc_research/scripts/update.sh [--commit <sha>] [--dry-run]
#   cd doc_research && pixi run update
#
...
# ROOT is the repository; the notebook itself is ROOT/doc_research. Manifest
# paths, and everything below, are relative to the repository.
...
command -v python3 >/dev/null 2>&1 && python3 doc_research/scripts/check.py || true
