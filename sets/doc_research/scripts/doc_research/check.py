...
It also reports which agent-written code Erin has not checked yet.

    pixi run check              # verify
    pixi run check --write      # verify, rebuild include lists, refresh INDEX.md
...
MANIFEST = ROOT / ".instructions" / "manifest"
BASELINE = ROOT / ".instructions" / "baseline"
...
def check_entries(report: Report) -> list[be.Entry]:
    ...
    stale = be.up_to_date()
    for name in stale:
        report.error(f"{name} is out of date — run `pixi run entries`")
...
def check_week_files(report: Report) -> list[be.Entry]:
    ...
    if be.WEEK_DIR.is_dir() and not (be.WEEK_DIR / wanted).exists():
        report.note(f"no summary for last week ({wanted[:-4]}) — `pixi run week` starts one")
    return weeks
...
def scan_markers(report: Report) -> tuple[list[Marker], int]:
    """Erin's habit, automated: which agent-written code has she not read yet?"""
    unchecked: list[Marker] = []
    checked = 0
    ours = (ROOT / "scripts" / "doc_research").resolve()
...
def check_template_drift(report: Report) -> None:
    ...
    for path in sorted(BASELINE.rglob("*")):
        ...
        local = ROOT / path_rel
        if not local.exists():
            report.warn(f"{path_rel}: shipped file is missing — `pixi run update` restores it")
...
def main() -> int:
    ...
    if unchecked or checked:
        print(f"agent-written code: {len(unchecked)} unchecked, {checked} checked by Erin")
