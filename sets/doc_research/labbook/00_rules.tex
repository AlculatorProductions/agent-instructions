...
them, and the figures that resulted. It is written by the agent, from Erin's handwritten
calculations and from the code she is working on.
...
\texttt{pixi run entries}. There are no topical chapters, and nothing is ever moved.
...
\texttt{pixi run check} verifies, and then \verb|\entryhead|:
...
\begin{enumerate}
  \item \textbf{What was asked.} One or two sentences, in Erin's terms.
  \item \textbf{The calculation}, inside a \texttt{transcription} environment, reproduced as
        written --- her notation, her ordering, her conventions. Nothing tidied, nothing
...
\begin{itemize}
  \item Erin's notation wins. Where a source uses a different convention, the entry states the
...
If the agent believes it has found a mistake in Erin's mathematics, it says so \emph{after}
transcribing faithfully, never by editing. The flag, its evidence, the proposed fix and
\textbf{Erin's decision} are recorded in the entry with \verb|\flag|:
...
Figures come from Erin's own plotting code, in her own style. Every figure file has a sibling
\texttt{figures/<name>.md} explaining in plain language what it shows, what the axes are, and what
produced it. \texttt{pixi run check} fails on a figure without one.
